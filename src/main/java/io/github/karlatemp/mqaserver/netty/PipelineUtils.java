...
import io.github.karlatemp.mqaserver.protocol.*;
import io.github.karlatemp.mqaserver.utils.HandlerBoss;
...
public class PipelineUtils {
    ...
    public static final ChannelInitializer<Channel> SERVER_CHILD = new ChannelInitializer<Channel>() {
        ...
        protected void initChannel(Channel ch) throws Exception {
            ...
            ch.pipeline().addAfter(FRAME_DECODER, PACKET_DECODER, new MinecraftDecoder(Protocol.HANDSHAKE, true, ISystemCore.getInstance().getProtocolVersion()));
            ch.pipeline().addAfter(FRAME_PREPENDER, PACKET_ENCODER, new MinecraftEncoder(Protocol.HANDSHAKE, true, ISystemCore.getInstance().getProtocolVersion()));
            ch.pipeline().addBefore(FRAME_PREPENDER, LEGACY_KICKER, legacyKicker);
            ch.pipeline().get(HandlerBoss.class).setHandler(
                    new InitialHandler()
            );
    ...
    static {
        if (!PlatformDependent.isWindows() && Boolean.parseBoolean(System.getProperty("bungee.epoll", "true"))) {
            Log.getLogger().info("Not on Windows, attempting to use enhanced EpollEventLoop");

            if (epoll = Epoll.isAvailable()) {
                Log.getLogger().info("Epoll is working, utilising it!");
            } else {
                Log.getLogger().log(Level.WARNING, "Epoll is not working, falling back to NIO: {0}", Util.exception(Epoll.unavailabilityCause()));
            }
        }
    }
    ...
    public static final class Base extends ChannelInitializer<Channel> {
        ...
        public void initChannel(Channel ch) throws Exception {
            ...
            // TODO
            ch.pipeline().addLast(TIMEOUT_HANDLER, new ReadTimeoutHandler(100086, TimeUnit.MILLISECONDS));
